...
export function TableSocis(props) {
  const { users, updateUser, deleteUser, enviarMail } = props;
  ...
  const [state, dispatch] = React.useReducer(exampleReducer, {
    column: null,
    data: users, //no s'actualitza data quan hi ha un canvi de users
    direction: null,
  });
    ...
    >
      <Table.Header>
        <Table.Row>
          ...
          <Table.HeaderCell>Actiuu</Table.HeaderCell>
    ...
    <>
      <Table.Cell textAlign="right">
        {checkPermis("EnviarMails") && (
          <Button icon color="blue" onClick={() => enviarMail(user.id)}>
            <Icon name="mail" />
          </Button>
        )}
